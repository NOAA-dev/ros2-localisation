...
class SafetyStop(Node):
    def __init__(self):
        ...
        self.stop_pub = self.create_publisher(Twist, '/bumper_bot/cmd_vel_unstamped', 10)
        ...
        while not self.decrease_speed_client.wait_for_server(timeout_sec=1.0):
            self.get_logger().warn('Action /joy_turbo_decrease not available! Waiting..')
            time.sleep(2.0)

        while not self.increase_speed_client.wait_for_server(timeout_sec=1.0):
    ...
    def laser_callback(self, msg: LaserScan):
        ...
        if self.state != self.prev_state:
            ...
            if self.state == State.WARNING:
                ...
                self.decrease_speed_client.send_goal_async(JoyTurbo.Goal())
                self.stop_pub.publish(Twist())  # Publish zero velocity to stop the robot immediately
                self.zones.markers[0].color.a = 1.0
